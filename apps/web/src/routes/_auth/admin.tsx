          ...
          {(data ?? []).map((o) => (
            <li key={o.id} className="flex items-center justify-between py-2 text-sm">
              ...
              <Button
                ...
                onClick={() => actualizar.mutate({ id: o.id, activo: !o.activo })}
              >
        ...
        <ul className="divide-y">
          {(data ?? []).map((t) => (
            <li key={t.id} className="flex items-center justify-between py-2 text-sm">
              <span className={t.activo ? "" : "text-muted-foreground line-through"}>
                {t.nombre}
              </span>
              <Button
                type="button"
                ...
                size="sm"
                disabled={actualizar.isPending}
                onClick={() => actualizar.mutate({ id: t.id, activo: !t.activo })}
              >
      ...
      <CardContent>
        <ul className="divide-y">
          {(data ?? []).map((c) => {
            ...
            return (
              <li key={c.clave} className="flex items-center justify-between gap-2 py-2 text-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    disabled={actualizar.isPending}
                    onClick={() =>
                      actualizar.mutate({ clave: c.clave, valor: Number(valor) || 0 })
                    }
                  >
                    Guardar
                  </Button>
        ...
        >
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              value={form.role}
              onChange={(e) =>
                setForm((f) => ({ ...f, role: e.target.value as "operario" | "admin" }))
              }
            >
