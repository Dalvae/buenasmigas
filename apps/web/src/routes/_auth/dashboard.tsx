...
function RouteComponent() {
	...
	const [envasado, setEnvasado] = useState<EnvasadoFila[]>([
		{ ...envasadoVacio },
	]);
		...
		orpc.registros.crear.mutationOptions({
			...
			onError: (error) => {
				if (error instanceof ORPCError && error.code === "CONFLICT") {
					toast.error(
						error.message || "Ya existe un registro para esa fecha y turno.",
					);
				} else {
	...
	return (
		<div className="mx-auto max-w-3xl px-4 py-6">
			<h1 className="mb-4 font-bold text-2xl">Captura de turno</h1>
				...
				<Card>
					...
					<CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-3">
						...
						<div className="space-y-2">
							...
							<div className="flex h-8 items-center font-medium text-sm">
								{elaboracionPct}%
							</div>
						</div>
					</CardContent>
					...
					<CardContent className="space-y-3">
								...
								>
									<div className="space-y-1">
										...
										<select
											...
											onChange={(e) =>
												updateEnvasado(i, { tipoEnvasadoId: e.target.value })
											}
										>
											<option value="">Selecciona…</option>
											{tipos.map((t) => (
									...
									<div className="space-y-1">
										...
										<Input
											...
											value={fila.pedido}
											onChange={(e) =>
												updateEnvasado(i, { pedido: e.target.value })
											}
										/>
									</div>
									<div className="space-y-1">
										...
										<Input
											...
											value={fila.real}
											onChange={(e) =>
												updateEnvasado(i, { real: e.target.value })
											}
										/>
									</div>
									<Button
										...
										size="sm"
										onClick={() =>
											setEnvasado((rows) => rows.filter((_, idx) => idx !== i))
										}
									>
										Quitar
									</Button>
						...
						<div className="flex items-center justify-between">
							<Button
								...
								size="sm"
								onClick={() =>
									setEnvasado((rows) => [...rows, { ...envasadoVacio }])
								}
							>
								Agregar
							</Button>
								...
								>
									<div className="space-y-1">
										...
										<Input
											...
											onChange={(e) =>
												updatePnc(i, { descripcion: e.target.value })
											}
										/>
									</div>
									<div className="space-y-1">
										...
										<Input
											...
											value={fila.unidades}
											onChange={(e) =>
												updatePnc(i, { unidades: e.target.value })
											}
										/>
									</div>
									<div className="space-y-1">
									...
									<div className="space-y-1">
										...
										<Input
											...
											value={fila.bandejas}
											onChange={(e) =>
												updatePnc(i, { bandejas: e.target.value })
											}
										/>
									</div>
									<Button
										...
										size="sm"
										onClick={() =>
											setPnc((rows) => rows.filter((_, idx) => idx !== i))
										}
									>
										Quitar
									</Button>
